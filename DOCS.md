# Documentation

Welcome to git-contributor-stats documentation!

## 📚 Documentation Files

### For Users

1. **[README.md](./README.md)** - Complete documentation
   - Full feature list
   - CLI reference
   - API documentation
   - Configuration options

2. **[QUICK-START.md](./QUICK-START.md)** - Get started fast
   - 30-second quickstart
   - 5 critical use cases with examples
   - Common commands
   - Real-world scenarios

3. **[QUICK-REFERENCE.md](./QUICK-REFERENCE.md)** - Command cheat sheet
   - One-page reference
   - Most common commands
   - All options explained
   - Bash aliases

### For Developers

4. **[TECHNICAL.md](./TECHNICAL.md)** - Technical details
   - Architecture overview
   - Tree-shaking & bundling
   - Version 2.0 refactoring
   - Development guide

---

## 🚀 Where to Start?

### "Just tell me what to do" (2 minutes)
1. Read the 30-second start in [QUICK-START.md](./QUICK-START.md#30-second-start)
2. Run: `git-contributor-stats --since 90.days --out-dir reports --md --html --charts`
3. Done!

### "Show me examples" (10 minutes)
1. Read [QUICK-START.md](./QUICK-START.md) - 5 use cases with code
2. Try 2-3 commands in your repo
3. Bookmark [QUICK-REFERENCE.md](./QUICK-REFERENCE.md)

### "I want all the details" (30 minutes)
1. Read [README.md](./README.md) - Complete documentation
2. Check [TECHNICAL.md](./TECHNICAL.md) - Architecture details
3. Explore the API

---

## 📖 Quick Links

### Common Tasks
- [Installation](./README.md#installation)
- [Basic Commands](./QUICK-START.md#-most-popular-commands)
- [Output Formats](./QUICK-REFERENCE.md#output-formats)
- [Use Cases](./QUICK-START.md#-5-critical-use-cases)

### Configuration
- [Time Filtering](./QUICK-REFERENCE.md#time-filtering)
- [Identity Management](./QUICK-START.md#5--identity-management)
- [GitHub Actions](./README.md#github-actions-integration)

### Advanced
- [Programmatic API](./README.md#programmatic-api)
- [Tree-Shaking](./TECHNICAL.md#tree-shaking--modularization)
- [Development](./TECHNICAL.md#development-guide)

---

## 🎯 The One Command You Need

```bash
git-contributor-stats --since 90.days --out-dir reports --md --html --charts
```

This generates everything: HTML dashboard, Markdown report, CSV export, and charts.

---

**Questions?** Check [README.md](./README.md) or open an issue on GitHub!

