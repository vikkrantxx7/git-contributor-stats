...
export async function seedBasicHistory(repoDir: string): Promise<void> {
  // initial commit
  ...
  await execa(
    'git',
    ['commit', '-m', 'Add features and src', '--author', 'Alice Developer <alice@example.com>'],
    { cwd: repoDir }
  );
  ...
  await execa(
    'git',
    ['commit', '-m', 'Improve app', '--author', 'Bob Contributor <bob@example.com>'],
    { cwd: repoDir }
  );
  ...
  await execa(
    ...
    [
      'commit',
      '-m',
      'Remove src.js and update changelog',
      '--author',
